...
3. **建置(`build` 指令)執行完全不經過 `system_command_runner`。**(已修正,見下方 Task 4 安全性回顧)最初的假設「`dpm` 整個程式從來沒有在任何地方對自己這個行程本身做權限提升」是**錯的**,只在 macOS 成立——Linux `--system` scope 下,`main.rs` 會呼叫 `sudo::escalate_if_needed()` 把整個 `dpm` process(不只是個別外部指令)重新以 root 執行,這是 pre-existing 行為,早於這個 plan。這代表如果 build 指令只是單純用 `std::process::Command` 直接執行、不做任何額外處理,Linux `--system` 下它會直接繼承 parent process 的 root 權限跑,違反 spec「build 指令永遠用當前呼叫者權限跑」的要求。這正是 Task 4 的安全性審查抓到並修正的落差:`install_source_package` 在 spawn build 指令前明確呼叫 `drop_privileges_for_build`,在 Linux root 下把 build 子行程主動降回 `SUDO_UID`/`SUDO_GID`(non-Linux/非-root 則是 no-op)。**`drop_privileges_for_build` 是這裡的關鍵安全機制,不是可有可無的防禦性程式碼**——之後若要移除或繞過它,必須先重新建立等價的權限保證,否則會悄悄重新引入「build 指令以 root 執行任意 shell 命令」的漏洞。(相應地,`clone_dir`/`out_dir` 這兩個 staging 目錄在 Linux root 下是由還沒降權的 parent process 建立、root 所有,降權後的 build 子行程原本寫不進去——`install_source_package` 在建立這兩個目錄後、spawn build 前,額外呼叫 `chown_dir_to_sudo_user` 把它們 chown 給同一個 `SUDO_UID`/`SUDO_GID`,讓降權後的子行程能實際寫入。)
