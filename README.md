...
| `build <package_name>`                                  | 把套件打包成 `Repo/<pkg>.dpm`(**Prebuilt** 發布流程的一步)                                                                             | `dpm-server build foo`                                     |
| `fix add <project_name> url <URL> [--file-name NAME]`  | 發布**Prebuilt** 版本:下載 `URL` 算 blake3 hash,寫進 `RepoInfo.json`;`URL` 必須是 `https://`,且必須通過作者簽章驗證     | `dpm-server fix add foo url https://example.com/foo.dpm`   |
